...
if __name__ == "__main__":
    ...
    for fname in tqdm.tqdm(fnames):
        ...
        saved_text = [json.dumps({"chunk_id": '_'.join([fname.replace(".txt", ''), str(i)]), "source": fname.strip(".txt"), "text": re.sub("\s+", " ", texts[i])}) for i in range(len(texts))]
        with open("./data/KB_books_v2/{:s}".format(fname.replace(".txt", ".jsonl")), 'w') as f:
            f.write('\n'.join(saved_text))