...
class RAGPipeline:
    ...
    def run(self, question, options=None, top_k_ret=5,
        ...
        context_input = self.contextualizer.build_input(question, retrieved_docs, options)
...
class NO_RAGPipeline:
    def __init__(self, LLM_name="mistralai/Mistral-7B-Instruct-v0.2", device="cpu"):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(LLM_name, trust_remote_code=True).to(device)
        self.model.eval()
        self.device = device

    def run(self, question, options, top_k_ret=0,
            max_new_tokens_gen=1000, do_sample_gen=False):
        ...
        with torch.no_grad():
            output = self.model.generate(**inputs, max_new_tokens=max_new_tokens_gen, do_sample=do_sample_gen, 
