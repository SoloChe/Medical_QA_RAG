...
def main(args):
    ...
    for count, sample in enumerate(eval_dataset):
        ...
        if args.rag and args.use_corrector:
            status, pred_raw, critique, revised_pred_raw, retrieved_docs = get_response(
                rag, question, options, top_k_ret=args.top_k_ret
            )
            if revised_pred_raw:
            ...
            pred_raw, retrieved_docs = get_response(
                rag, question, options, top_k_ret=args.top_k_ret
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_file",
        type=str,
        default="./data/processed/MedQA.jsonl",
        help="Path to the eval file",
    )
    parser.add_argument(
        "--log_dir", type=str, default="./logs_eval", help="Directory to save logs"
    )
    parser.add_argument(
        "--rag",
        type=str_to_bool,
        default=True,
        help="Use RAG pipeline if True, else use NO_RAG pipeline",
    )
    parser.add_argument(
        "--top_k_ret",
        type=int,
        default=10,
        help="Top K passages to retrieve in RAG pipeline",
    )
    parser.add_argument(
        "--use_corrector",
        type=str_to_bool,
    ...
    parser.add_argument(
        ...
        type=str_to_bool,
        default=False,
        help="Use ranker in RAG pipeline",
    )
