...
def train_lora_ppo(args):
    ...
    val_dataset = load_dataset("json", data_files=args.val_file, split="train").select(range(100))

    # Set up PPO Trainer
    ppo_config = PPOConfig(batch_size=args.batch_size, ppo_epochs=args.ppo_epochs, learning_rate=args.learning_rate)
    ppo_trainer = PPOTrainer(model, tokenizer, config=ppo_config)
    logger.info(f"PPO Trainer initialized with batch_size={args.batch_size}, ppo_epochs={args.ppo_epochs}")
    ...
    for step in range(args.max_steps):
        if step % args.save_steps == 0 and step > 0:
            ...
            # Save QA examples
            output_file = os.path.join(args.output_dir, f"qa_examples_step_{step}.jsonl")
            save_qa_examples(model, tokenizer, val_dataset, output_file, num_examples=5, logger=logger, wandb_run=wandb.run)
        ...
        logger.info(f"[INFO] Step {step+1}/{args.max_steps} - Avg Reward: {sum(rewards) / len(rewards):.4f}")
...
def main():
    ...
    parser.add_argument('--wandb_project', type=str, default='Medical_QA_LoRA_PPO', help='Wandb project name')
    parser.add_argument('--save_steps', type=int, default=500, help='Checkpoint save steps')
