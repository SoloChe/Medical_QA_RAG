...
class RAGPipeline:
    def __init__(self,
                 retriever_path="./data/medical_docs.txt",
                 model_name="./saved_models/lora_finetuned",
                 fact_checker_model="all-mpnet-base-v2",
                 summarizer_model="facebook/bart-large-cnn",
                 top_k=3,
                 device="cuda"):
        self.retriever = BM25Retriever(docs_path=retriever_path)
        self.contextualizer = Contextualizer(model_name=model_name, top_k=top_k)
        self.generator = Generator(model_name=model_name, device=device)
    ...
    def run(self, query, top_k=3, max_length=512, temperature=0.7, do_sample=True, summarize=True):
        ...
        response = self.generator.generate(context_input, max_length=max_length, temperature=temperature, do_sample=do_sample)
